"""Vitusapotek.no — JS-rendered SPA, uses Playwright headless browser."""
import re, time, json
...
def fetch_price(url):
    try:
        with sync_playwright() as p:
            ...
            if not pris:
                for sel in ["[class*='price']", "[class*='Price']", "[data-testid*='price']"]:
                    ...
                    if el:
                        raw = el.inner_text().replace("kr","").replace("Kr","").replace(",",".").strip()
        ...
        print(f"  [vitusapotek] error: {e}")
        return None, None

def run(products):
    results, resolved = [], {}
    for p in products:
        url = p.get("url_vitusapotek") or search_url(p["varenummer"])
        if not url:
            print(f"  [vitusapotek] no URL: {p['varenummer']}")
            continue
        if not p.get("url_vitusapotek"):
            resolved[p["varenummer"]] = url
        pris, lager = fetch_price(url)
        print(f"  [vitusapotek] {p['varenummer']}: {pris}")
